...
/**
 * J16/RTR-4 — a buffer outlives the run that owns it, <b>for every timeout</b>, by construction.
 *
 * <p>Complements {@code RunTraceRegistryTtlContractTest}, which pins the derivation against the
 * SHIPPED {@code application.yml} — the stronger check for today's config, and the one that
 * catches a retune of {@code timeout-ms}. This file guards the other half: that the FORMULA
 * holds across the range, so a future retune cannot land in a gap between the two sampled
 * points. Both were written independently, in parallel worktrees, on the same afternoon.
 *
...
class TraceBufferOutlivesItsRunTest {
    ...
    void aShortTimeoutStillKeepsFinishedStepsReadableForAHuman() {
        assertThat(InMemoryRunTraceRegistry.ttlFor(Duration.ofSeconds(30).toMillis()))
                ...
                .isEqualTo(InMemoryRunTraceRegistry.TTL_FLOOR);
    }
