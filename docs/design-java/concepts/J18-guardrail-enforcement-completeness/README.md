...
**State**: 🟡 **Mostly built** (2026-08-06) — GEC-1/GEC-2 (allowlist moved to the gateway boundary, closing the find_recent_committers bypass), GEC-3 (encoded-query values sanitised) and GEC-4 (reflection-driven escape test) landed. **Remaining: GEC-5** (audit the other two query languages) **and GEC-6** (deterministic fan-out cap, already partly covered by J13/ECI-4) · **Complexity**: Moderate ·
**Priority**: MEDIUM ·
