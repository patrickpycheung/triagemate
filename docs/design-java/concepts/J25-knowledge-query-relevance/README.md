...
**State**: 🟡 Mostly built — **KQR-2 landed 2026-08-06** (relevance floor; scores system and symptom separately and takes the better, after an exact-match first cut dropped the demo's own runbook). **Remaining: KQR-4 only**, which is the same rule as J14/FRI-5 and must be implemented once, not twice. (2026-08-05) — **KQR-1** (siteSearch + symptom-as-written) and
