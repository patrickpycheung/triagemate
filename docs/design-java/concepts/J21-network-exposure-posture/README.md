...
**State**: 🟡 **Mostly built** (2026-08-06) — NEP-1 (loopback bind) and NEP-2 (required non-safelisted header on the mutating endpoint, sent by the UI) landed. **Remaining: NEP-3** (make the posture visible in the banner) **and NEP-4** (the documented escape hatch) · **Complexity**: Simple · **Priority**: MEDIUM ·
