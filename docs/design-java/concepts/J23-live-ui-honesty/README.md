...
**State**: 🟢 **Built** — all four LUH rules (2026-08-06) — LUH-1 (caption owned by the state, settled at the transition), LUH-2 (a finished live run stops claiming steps are arriving) and LUH-4 (one branch per engine; a degraded run is no longer described as offline, and no engine value inherits a network claim through an else) landed. **LUH-3 landed 2026-08-06** — connector modes are served on /api/ui-config (additive DTO, existing field names unchanged) and the connector chip renders as the live shell opens. The ENGINE chip is deliberately still withheld until the first step arrives, because guessing it is LUH-4 in a different costume. **All four rules built.** · **Complexity**: Simple ·
**Priority**: LOW ·
