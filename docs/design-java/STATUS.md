...
| J31 | allowlist-sweep-outcome | Moderate | 🟠 **Evolving** (Round 4, 2026-08-06). Derived from cheungp's `Patrick_gitlab-update_allowed-projects.md`, which is **already implemented** (J30/GEB-1) — this is what that left behind. Now the allowlist holds TWO entries, the sweep in `DeterministicDiagnosisEngine` disagrees with rules already converged: the `break` on failure **violates J30/GEB-2** ("a project that cannot resolve is a *skipped* project" — and GEB-2's own verification row requires the sweep to continue), and `codeSearchFailed` records the LAST attempt, so a real search that ran and found nothing is overwritten by the demo entry's 404. Fix: `continue`; a structured 4-state outcome (hits / all-empty / **partial** / none-searched) carrying per-project failures. **Not converged** — ASO-4 (the trace state for a partly-failed sweep) is an open ADM-2 needing J11's state vocabulary | J30, J14, J6 (amends J30/GEB-2, J30/GEB-3, J14/FRI-5) |
