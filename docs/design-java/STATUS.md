...
| J18 | guardrail-enforcement-completeness | Simple | 🟡 **Mostly built 2026-08-06 — GEC-1/2/3/4 landed; GEC-5/6 remain.** A bound is owned by the boundary, not the caller: the GitLab allowlist FND-38 added to `search_code` is bypassed by `find_recent_committers`, and ServiceNow encoded-query values are unconstrained | J2, J5, J6, J8 (amends J8, J5) |
| J19 | instruction-config-fidelity | Simple | 🟡 **Mostly built 2026-08-06 — ICF-1/2/3/5 landed; ICF-4 remains.** Every bound the prompt states derives from the enforced `TriageProperties` value — no second hardcoded `prod`, budget disclosed up front, exhaustion phrased globally (completes FND-60's discipline) | J2, J8 (amends J8, J2) |
