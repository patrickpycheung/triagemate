...
| J16 | run-trace-registry-lifecycle | Moderate | 🟡 **Partly built 2026-08-06 — RTR-4 (TTL derived from the run bound) landed; RTR-1/2/3/5 remain.** A live buffer belongs to the run in flight, not to whoever last sent a header for that incident — fixes waiter aliasing onto stale/poller-owned runs, and derives the TTL from `timeout-ms` | J1, J10, J11 (amends J11) |
