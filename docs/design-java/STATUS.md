...
| J30 | gitlab-estate-binding | Simple | 🟡 **Partly built 2026-08-06 — GEB-2/3/4 landed (its J14/FRI-5 prerequisite is now satisfied); only GEB-1 is genuinely blocked on one fact from a network that can reach GitLab.** **Field-reported** (cheungp, `934fb0a`, live GitLab). `triage.gitlab.allowed-projects` holds only the OFFLINE DEMO project `order-payments/payment-service`, which does not exist in the real estate, so every real-mode code search 404s. **Gated on J14/FRI-5**: the engine sweeps the allowlist calling `searchCode` per entry and that call still propagates, so adding entries before FRI-5 lands makes the demo *less* reliable, not more. The real project path cannot be discovered from a dev machine — `gitlab.cd.auspost.com.au` returns an identical 403 from `awselb/2.0` with token, without token, and on the web root, i.e. a network perimeter block, NOT a bad credential | J6, J3, J14 (amends J8, J7) |
