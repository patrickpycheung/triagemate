...
> 🔵 Proposed (design not yet written). **Every card J1–J31 is design-converged** — each has
> Essence, a Design section with named items, Verification and Out-of-scope, audited
> 2026-08-06. (J31 is 🔵 by BUILD state, not by design state: its design is written and
> converged, it simply has no implementation yet.) So a 🔴 here means "nobody has implemented this yet", NOT "the design is still
...
| J31 | allowlist-sweep-outcome | Simple | 🔵 **Proposed** (2026-08-06). Derived while checking cheungp's `Patrick_gitlab-update_allowed-projects.md` against the tree — that file is **already implemented** (J30/GEB-1); this is the defect it left behind. Now that the allowlist holds TWO entries, the sweep in `DeterministicDiagnosisEngine` mishandles a multi-entry list: a failing entry `break`s the loop, so a phantom project ranked first stops the real one from ever being searched (**ASO-A**); and `codeSearchFailed` records the LAST attempt, so a successful-but-empty real search is overwritten by the demo entry's 404 and reported as *could not search* — the FND-8 class, and exactly the distinction **J30/GEB-3** asked for but specified only for a single attempt (**ASO-B**). Fix: skip-and-continue, aggregate the outcome across attempts, and keep the offline fixture out of real-mode sweeps | J30, J14, J6 (amends J30) |
