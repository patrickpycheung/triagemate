...
> 2026-08-06. **J31 (🟠) and J32 (🔵) are the exceptions and are genuinely not converged**:
> J31 has one open ADM-2 (the trace state for a partly-failed sweep), and J32 is a fork not
> yet decided. Do not hand either to implementation on the strength of the J1–J30 audit. So a 🔴 here means "nobody has implemented this yet", NOT "the design is still
...
| J16 | run-trace-registry-lifecycle | Moderate | 🟢 **Built** (2026-08-06). RTR-1/2/4/5 plus RTR-3 both halves — the deferred client-side twin landed once its worktree conflict cleared. RTR-1/2/4/5 + RTR-3 server-side landed: every run registers a buffer (server-minted runId when the caller sent none), `aliasTo` binds runId→runId and the `currentRunIdByIncident` index is **deleted**, ordering is register→publish→putIfAbsent so the register race is eliminated rather than narrowed, and TTL derives from `timeout-ms` (a link, not a retune — 5 min at the shipped value, pinned by a test). **Open**: RTR-3's client-side twin in `index.html`, deferred because another worktree held that file | J1, J10, J11 (amends J11) |
| J17 | poller-completion-semantics | Moderate | 🟢 **Built** — all five PCS rules landed 2026-08-06. K1 "completed" becomes diagnosed AND delivered AND not-already-done-by-another-trigger, with a no-LLM redelivery queue — today a failed writeback marks the incident done and its only external output is lost permanently | J1, J5, J10 (amends J10, J1) |
| J18 | guardrail-enforcement-completeness | Simple | 🟢 **Built** — all six GEC rules landed 2026-08-06 (the Confluence check found a real backslash hole, not a no-op). A bound is owned by the boundary, not the caller: the GitLab allowlist FND-38 added to `search_code` is bypassed by `find_recent_committers`, and ServiceNow encoded-query values are unconstrained | J2, J5, J6, J8 (amends J8, J5) |
| J19 | instruction-config-fidelity | Simple | 🟢 **Built** — all five ICF rules landed 2026-08-06. Every bound the prompt states derives from the enforced `TriageProperties` value — no second hardcoded `prod`, budget disclosed up front, exhaustion phrased globally (completes FND-60's discipline) | J2, J8 (amends J8, J2) |
| J20 | startup-truth-and-validation | Moderate | 🟢 **Built** — all six STV rules landed 2026-08-06. Boot validates what the run will need (LLM config, nullable allowlists) and the banner reports the *effective* engine, not configured intent — the FND-49/FND-36 class, re-opened | J1, J2, J8 |
| J21 | network-exposure-posture | Simple | 🟢 **Built** — all four NEP rules landed 2026-08-06. The mutating endpoint is reachable from the whole LAN with no auth while writeback is on by default: loopback bind + a required non-safelisted header that forces an unanswered CORS preflight | J1, J5, J7, J11 (amends J1, J7) |
...
| J30 | gitlab-estate-binding | Simple | 🟢 **Built** (2026-08-06) — GEB-1 unblocked the same day by cheungp supplying the real path; GEB-2/3/4 landed. **Field-reported** (cheungp, `934fb0a`, live GitLab). `triage.gitlab.allowed-projects` holds only the OFFLINE DEMO project `order-payments/payment-service`, which does not exist in the real estate, so every real-mode code search 404s. **Gated on J14/FRI-5**: the engine sweeps the allowlist calling `searchCode` per entry and that call still propagates, so adding entries before FRI-5 lands makes the demo *less* reliable, not more. The real project path cannot be discovered from a dev machine — `gitlab.cd.auspost.com.au` returns an identical 403 from `awselb/2.0` with token, without token, and on the web root, i.e. a network perimeter block, NOT a bad credential | J6, J3, J14 (amends J8, J7) |

| J31 | allowlist-sweep-outcome | Moderate | 🟢 **Built** (2026-08-06). The sweep now reports what it actually did. `break`→`continue` restores **J30/GEB-2** (a failed project is *skipped*, not the end of the run — its own verification row already said so); `codeSearchFailed` is replaced by `SweepOutcome`, five states where there were two, and failures carry the **project** (the exception only carries the system, so two 404s were indistinguishable). Per-attempt trace rows, each `DONE` or `FAILED`. Prerequisite **FND-89** fixed with it: `emitStep` hardcoded `DONE`, so no row on this engine could resolve `FAILED` and a degraded run rendered like a clean one — that also covered Confluence, Sumo and recentCommitters, not just GitLab | J30, J14, J6 (amends J30/GEB-3; restores GEB-2) |
| J32 | allowlist-eligibility-vs-authorization | Moderate | 🔵 **Proposed** (2026-08-06). Split out of J31's Round 3 conflict pass. `triage.gitlab.allowed-projects` answers two questions with one list — *may we call this project?* (a security bound enforced in `TriageMateTools` and `RealGitLabGateway`) and *is it worth calling for this incident?* (sweep candidates via `rankAllowlist`). J30/GEB-1 deliberately holds an entry correct for the first and impossible for the second. Deciding this touches ADK tool validation, the ADK instruction text (J19), banner logic and fixtures — which is why it is NOT in J31 | J6, J30, J31, J20, J8, J19 |
