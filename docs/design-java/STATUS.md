...
| J28 | precedent-grounded-cause-resolution | Moderate | 🔵 Proposed — from DDS `cause-and-resolution-sections`. Two new report components that **quote** what past incidents did rather than **assert** what is happening now: abstention legal and expected, no percentages, remediation verbs a closed enum. Both its blockers (J26, J27) are now resolved | J4, J5, J7, J26, J27 |
| J29 | log-line-fidelity | Simple | 🔴 Designed, not built — HIGH. **Field-reported** (cheungp, `6cc2f1c`, live Sumo). `loglevel` is **absent** on every real row so every log arrives at level `""`: the ERROR filter matches nothing, both GitLab steps skip, and — not in the report — every log-derived candidate silently drops from the 0.70 tier to 0.45. The token regex separately picks `GNAF_FRONTAGE` (a SQL data value) over `DataIntegrityViolationException`. Fourth mock-populates-it/real-doesn't finding after FND-47, FND-61, J24 | J6, J3, J22 (amends J6, J13) |
