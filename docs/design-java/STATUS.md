...
| J17 | poller-completion-semantics | Moderate | 🟡 **Mostly built 2026-08-06 — PCS-1/2/3/5 landed; only PCS-4 (cross-trigger) remains.** K1 "completed" becomes diagnosed AND delivered AND not-already-done-by-another-trigger, with a no-LLM redelivery queue — today a failed writeback marks the incident done and its only external output is lost permanently | J1, J5, J10 (amends J10, J1) |
| J18 | guardrail-enforcement-completeness | Simple | 🟡 **Mostly built 2026-08-06 — GEC-1/2/3/4 landed, GEC-5 (Sumo half) landed; only the Confluence one-look check remains.** A bound is owned by the boundary, not the caller: the GitLab allowlist FND-38 added to `search_code` is bypassed by `find_recent_committers`, and ServiceNow encoded-query values are unconstrained | J2, J5, J6, J8 (amends J8, J5) |
...
| J20 | startup-truth-and-validation | Moderate | 🟡 **Mostly built 2026-08-06 — STV-1/3/5 landed; STV-2/4/6 remain.** Boot validates what the run will need (LLM config, nullable allowlists) and the banner reports the *effective* engine, not configured intent — the FND-49/FND-36 class, re-opened | J1, J2, J8 |
| J21 | network-exposure-posture | Simple | 🟡 **Mostly built 2026-08-06 — NEP-1/2/3 landed; only NEP-4 remains.** The mutating endpoint is reachable from the whole LAN with no auth while writeback is on by default: loopback bind + a required non-safelisted header that forces an unanswered CORS preflight | J1, J5, J7, J11 (amends J1, J7) |
...
| J30 | gitlab-estate-binding | Simple | 🔴 Designed, not built — HIGH, **one input blocked**. **Field-reported** (cheungp, `934fb0a`, live GitLab). `triage.gitlab.allowed-projects` holds only the OFFLINE DEMO project `order-payments/payment-service`, which does not exist in the real estate, so every real-mode code search 404s. **Gated on J14/FRI-5**: the engine sweeps the allowlist calling `searchCode` per entry and that call still propagates, so adding entries before FRI-5 lands makes the demo *less* reliable, not more. The real project path cannot be discovered from a dev machine — `gitlab.cd.auspost.com.au` returns an identical 403 from `awselb/2.0` with token, without token, and on the web root, i.e. a network perimeter block, NOT a bad credential | J6, J3, J14 (amends J8, J7) |
